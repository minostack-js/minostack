...
> Status 2026-09-09: **Blockers fixed + D2 DONE, ship-ready (no publish)** — license standardized to Apache-2.0 (13 manifests), mino changeset added, `compose` strict contract enforced (no silent auto-advance → 500; 3 tests updated), coverage at uniform 90% (95.4% stmts / 90.2% branches / 98.0% funcs / 96.5% lines, gate restored to 90×4), `bench:matrix` real-socket harness (11 scenarios × 3 fw × 3-run medians + 60s sustained + post-GC heap) run on Node 22 + 24 with honest verdict (D2 bar NOT met: Express leads 11/11 on Node 24; Node 22 anomaly flagged, needs isolated re-run). Gates: mino 265/265, `pnpm check` 17/17, `build` 9/9. Remaining: isolated re-run of Node 22 anomaly, `changeset publish` (trusted env only).
> Status 2026-09-09 pm: **Fastify v5 added as 4th benchmark framework** — `bench:fastify` (fetch via `app.inject`), `bench:network` + `bench:matrix` real sockets (`app.listen`), full 11-scenario × 4-fw × 3-run matrix re-run on Node 22 + 24 + 60s sustained. Result: Fastify leads all 24 cells (~1.5–2×); D2 bar still not met; Node 22 Express anomaly reproduced (consistent, still unexplained). README tables updated to 4 fw. Gates re-verified below, no publish.
